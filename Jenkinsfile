        ...
        stage('Checkout and run devprofiler') {
            steps {
                script {
                    ...
                    sh 'sudo -s apt install libssl1.1_1.1.1f-1ubuntu2_amd64.deb -y'
                    sh 'wget https://github.com/Alokit-Innovations/dev-profiler/releases/download/v0.2.0/devprofiler.deb'
                    sh 'sudo -s apt install devprofiler.deb -y'
                    sh 'sudo -s devprofiler -- jenkins "${CLONE_DIR}" "${REPO_FULL_NAME}"'
                    sh 'timestamp=$(date +%s)'
                    sh 'filename="${timestamp}-devprofile.jsonl.gz"'
                    sh 'sudo -s mv devprofile.jsonl.gz "${filename}"'
                    sh 'sudo -s curl -F "file=@${filename}"  https://gcscruncsql-k7jns52mtq-el.a.run.app/upload'
                }
            }
        }
    }
